...
namespace DotNetShipping.ShippingProviders
{
    ...
    public class UPSProvider : AbstractShippingProvider
    {
        ...
        private void parseRatesResponseMessage(string response)
        {
            ...
            //check for errors
            var errors = xDoc.SelectNodes("/RatingServiceSelectionResponse/Response/Error");
            if (errors.Count > 0)
            {
                foreach (XmlNode node in errors)
                {
                    AddError(
                    new ShippingResponseError
                             {
                                 Description = node.SelectSingleNode("ErrorDescription").InnerText,
                                 Code = node.SelectSingleNode("ErrorCode").InnerText,
                                 Severity = node.SelectSingleNode("ErrorSeverity").InnerText
                             }
                    );
                }
            }
